...
export default function DashboardPage() {
  ...
  const todayQuestion = !isLoggedIn
    ? t.dash.guestToday
    : isTeacher
      ? t.dash.todayQuestionTeacher
      : t.dash.todayQuestion;
  const aiTip = !isLoggedIn
    ? t.dash.guestTip
    : isTeacher
      ? t.dash.aiTipTeacher
      : t.dash.aiTip;
  ...
  return (
    <AppShell>
      <PageMain>
              ...
              <div className="flex flex-wrap items-center justify-center gap-3 lg:justify-start">
                {!isLoggedIn ? (
                  <>
                    ...
                    >
                      <Icon name="login" /> {t.platform.login}
                    </button>
                    <Link
                      href="/news"
                    ...
                    >
                      <Icon name="newspaper" /> {t.navExtra.news}
                    </Link>
                  </>
                ) : (
                  <>
        ...
        {!isLoggedIn ? (
          <section className="cloud-shadow rounded-[1.75rem] border border-outline-variant/40 bg-white p-6 sm:p-8">
            <div className="mb-4 flex size-12 items-center justify-center rounded-2xl bg-primary-fixed text-primary">
              <Icon name="lock" filled />
            </div>
            <h2 className="font-headline-md text-headline-md mb-2 text-on-surface">
              {t.dash.guestLockedTitle}
            </h2>
            <p className="font-body-md text-body-md mb-6 max-w-2xl text-on-surface-variant">
            ...
            <div className="flex flex-wrap gap-3">
              <button
                type="button"
                onClick={() => openLogin("/")}
                className="btn-cute inline-flex items-center gap-2 rounded-full bg-primary px-5 py-3 font-label-md text-label-md text-on-primary"
              >
                <Icon name="login" /> {t.platform.login}
              </button>
              <Link
                href="/news"
                className="inline-flex items-center gap-2 rounded-full border border-outline-variant bg-surface-container-low px-5 py-3 font-label-md text-label-md text-primary"
              >
                <Icon name="newspaper" /> {t.navExtra.news}
              </Link>
            </div>
          ...
          <>
            ...
            <div className="mb-10 grid grid-cols-2 gap-3 sm:gap-4 lg:grid-cols-4">
              {[
                ...
                >
                  <div
                    className={`mb-3 inline-flex size-10 items-center justify-center rounded-2xl ${s.chip}`}
                  >
                  ...
                  <div className="mt-auto pt-2 font-label-sm text-[11px] text-on-surface-variant">
                    {s.sub}
                  </div>
                </div>
              ))}
